...
def chat_ui():
    ...
    user_q = st.text_input("Ask about stock, SKU, supplier, price…", key="chat_input")
    if user_q:
...
if choice == "Dashboard":
    ...
    st.markdown('<div class="paper">', unsafe_allow_html=True)
    tiles = st.columns([1.3, 1.3, 1.3, 1.3, 3])
    with tiles[0]: metric_tile("Stock Items", len(products), "distinct products")
